...
def test_get_last_sources_accumulates_across_repeated_calls_to_same_tool(
    ...
    sources = manager.get_last_sources()
    assert sources == [
        {"text": "Course A - Lesson 1", "link": "https://example.com/intro-to-rag/lesson-2"},
        {"text": "Course B - Lesson 3", "link": "https://example.com/intro-to-rag/lesson-2"},
    ]
...
def test_execute_tool_with_unknown_name_does_not_affect_accumulated_sources(mock_vector_store):
    manager = ToolManager()
    manager.register_tool(CourseSearchTool(mock_vector_store))
